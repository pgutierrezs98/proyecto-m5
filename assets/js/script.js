...
//Agregar tareas
gestorTareas.agregarTarea(new Tarea(1, 'Tarea 1', true, Date.now(), Date.now()));
gestorTareas.agregarTarea(new Tarea(2, 'Tarea 2', true, Date.now(), Date.now()));
gestorTareas.agregarTarea(new Tarea(3, 'Tarea 3', false, Date.now(), Date.now()));
...
formularioTarea.addEventListener('submit', (event) => {
    ...
    // Si es que se selecciona una fecha limite en el input, la convertimos a milisegundos, si no -> devolvemos undefined
        ...
        gestorTareas.agregarTarea(new Tarea(Date.now(), descripcion, false, Date.now(), tiempoLimite),
        );
